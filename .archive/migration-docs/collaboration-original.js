...
class CollaborationManager {
    ...
    async handleJoinProject(socket, { projectId, userId, username, displayName }) {
        try {
            ...
            console.log(`🔐 Creating session for socket ${socket.id}:`, {
                userId: user.id,
                projectId: actualProjectId,
                username: user.username,
                sessionId: sessionId
            ...
            console.log(`✅ Session created. Total sessions: ${this.userSessions.size}`);
            ...
            // Notify other users about the new user AND send them updated active users list
            ...
            // Broadcast updated active users list to all OTHER users in the project
            socket.to(`project_${actualProjectId}`).emit('active_users_update', {
                users: activeUsers
            });
    ...
    handleDisconnect(socket) {
        ...
        // Debug: Show all sessions before disconnect
        console.log(`📋 Sessions before disconnect: ${this.userSessions.size}`);
        
        const session = this.userSessions.get(socket.id);
        if (session) {
            console.log(`🔍 Found session for disconnecting socket:`, {
                socketId: socket.id,
                userId: session.userId,
                username: session.username,
            ...
            console.log(`⚠️ No session found for disconnecting socket: ${socket.id}`);
        }
        
        console.log(`📋 Sessions after disconnect: ${this.userSessions.size}`);
    }
    
    async removeUserFromProject(socket, projectId, session) {
        try {
            console.log(`🔐 Removing user from project - Socket: ${socket.id}, User: ${session.username}, Project: ${projectId}`);
            ...
            socket.to(`project_${projectId}`).emit('active_users_update', {
                users: activeUsers
            });
    ...
    async handleSyncCheck(socket, { projectId, sequenceNumber, stateHash, timestamp }) {
        ...
        console.log('📋 Current sessions:', this.userSessions.size);
        for (const [sid, sess] of this.userSessions.entries()) {
            console.log(`  - Socket ${sid}: User ${sess.username} (${sess.userId}) in project ${sess.projectId}`);
        }
