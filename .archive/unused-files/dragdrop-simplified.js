...
class SimplifiedDragDropManager {
    ...
    async onDrop(e) {
        ...
        const dropPos = [
            (e.clientX - rect.left) / window.app.graphCanvas.viewport.scale + window.app.graphCanvas.viewport.offset[0],
            (e.clientY - rect.top) / window.app.graphCanvas.viewport.scale + window.app.graphCanvas.viewport.offset[1]
        ];
    ...
    async processFiles(files, dropPos) {
        ...
        try {
            ...
            // PHASE 4: Upload files in background
            console.log('📊 Phase 4: Starting background uploads...');
            this.startBackgroundUploads(fileInfos);
            
            // PHASE 5: Sync server state
            console.log('📊 Phase 5: Scheduling server state sync...');
            this.scheduleServerSync(nodes);
    ...
    /**
     * Schedule server state synchronization
     */
    scheduleServerSync(nodes) {
...
// Export for use
window.SimplifiedDragDropManager = SimplifiedDragDropManager;